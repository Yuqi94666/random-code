'use strict';
window.DCP16910DeviceCohort = 'popularDevices'; // popularDevices || appleDevices
    ...
    generateDeviceCard: function (d) {
        ...
        return `
            ...
            <span class="mth">per month</span>
                    ...
                    <div class="primaryBtn">${d.cta ? d.cta : `Show now`}</div>
                </div>
            </div>
        </a>
    `
            ...
            <a class="planCard itemCard ${exclusiveClass}" title="${p.title}" href="${p.href}" aria-hidden="false">
                <div class="plan-item">
                    ...
                    <div class="plan-text">
                        ...
                        <p class="plan-link">${p.cta ? p.cta : `Show now`}</p>
                    </div>
                </div>
            </a>
                `
...
var DCP16910Fn = {
    ...
    <div id="DCP16910Wrapper">
      ...
      @media (max-width: 768px) {
        ...
        #DCP16910Wrapper_tabContents { min-height: 570px; }
    ...
    buildCohort: function (whitelist, apiMap, useFallback) {
        return whitelist.reduce(function (acc, cfg) {
            if (useFallback) {
                acc.push({ deviceName: cfg.name, brand: cfg.brand, image: cfg.image, recurringCharge: cfg.recurringCharge, wasPrice: cfg.wasPrice, href: cfg.href, cta: cfg.cta || null });
                return acc;
            }
            ...
            acc.push({
                ...
                image: api.imageUrl || '',
                recurringCharge: api.discountedRecurringCharge || api.recurringCharge || 0,
