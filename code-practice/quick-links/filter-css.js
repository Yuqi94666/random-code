...
let DCP15834OBJ = {
    ...
    buildNewCarousel: function () {
        // document.querySelector('#primaryPlans .slick-track').classList.add('section-hidden');
        ...
        this.loadSlickCarousel().then(() => {
            ...
            // Get the slick-slider element
            const slickSlider = $('#primaryPlans .slick-slider');
            if (slickSlider.length === 0) return;

            // Clone the slick-slider element
            const clonedSlider = slickSlider.clone(true);

            // Add ID to the cloned slider for easier targeting
            clonedSlider.attr('id', 'newPlans');

            // Replace the original slick-slider with the cloned one
            slickSlider.replaceWith(clonedSlider);
    ...
    setupNewCarouselEvents: function () {
        ...
        // stet up click event for the new carousel
        $('#newPlans [role="contentinfo"] button').click(function () {
        ...
        // set up 
        $('html').on('click', '#newPlans #prepaidplusmaxspeed', function () {
            document.querySelector('#primaryPlans #prepaidplusmaxspeed')?.click();
        });
        $('html').on('click', '#newPlans #databank', function () {
            document.querySelector('#primaryPlans #databank')?.click();
        });
        $('html').on('click', '#newPlans #autorecharge', function () {
            document.querySelector('#primaryPlans #autorecharge')?.click();
        });
        $('html').on('click', '#newPlans #zone1', function () {
    ...
    applyChanges: function (el) {
        try {
            ...
            if (!this.isMobile()) {
                ...
                // const aa = document.querySelector('#primaryPlans .slick-slider ');
                // console.log(aa);
                // aa.classList.add('section-hidden');
    ...
    applyFilters: function (activeFilters) {
        if (this.isMobile()) {
            ...
            if (this.newCarouselBuilt) {
                this.applyDesktopFiltersNewCarousel(activeFilters);
            } else {
    ...
    applyDesktopFiltersNewCarousel: function (activeFilters) {
        const allPlanContainers = document.querySelectorAll('#newPlans .slick-slide');
        const dayFilters = {
            '7Day': ['7 day expiry'],
            '28Day': ['28 day expiry'],
            'longDays': ['185 day expiry', '365 day expiry']
        ...
        allPlanContainers.forEach(dtContainer => {
    ...
    updateCounter: function () {
        ...
        if (this.isMobile()) {
            ...
            visiblePlans = Array.from(document.querySelectorAll('#newPlans .slick-slide:not(.section-hidden) [data-testid*="plan-card-PRD_PREPAID"]'));
            totalPlans = Array.from(document.querySelectorAll('#newPlans .slick-slide [data-testid*="plan-card-PRD_PREPAID"]'));
        } else {
